...
WHERE v.fecha_venta = @FECHA_ACTUAL AND HOUR(v.hora) < HOUR(@HORA_ACTUAL);
...
WHERE
    ...
    b.causa <> 'A1'  AND HOUR(v.hora) < HOUR(@HORA_ACTUAL);
...
SELECT
    CONCAT_WS(
        ...
        CONCAT('Contado efectivo $ ',FORMAT(IFNULL(SUM(CE),0.0)/1.16,2)),
        CONCAT('Contado tarjeta $ ',FORMAT(IFNULL(SUM(CT),0.0)/1.16,2)),
        CONCAT('Contado resto $ ',FORMAT(IFNULL(SUM(CR),0.0)/1.16,2)),
        CONCAT('Enganches $ ',FORMAT(IFNULL(SUM(ENG),0.0)/1.16,2)),
        CONCAT('Cobros tienda $ ',FORMAT(IFNULL(SUM(ABOT),0.0)/1.16,2)),
        CONCAT('Cobros telefonica $ ',FORMAT(IFNULL(SUM(ABOG),0.0)/1.16,2)),
        CONCAT('Cobros presencial $ ',FORMAT(IFNULL(SUM(ABOM),0.0)/1.16,2)),
        CONCAT('Cobros Extra-Jud. $ ',FORMAT(IFNULL(SUM(ABOE),0.0)/1.16,2)),
        CONCAT('Cobros Pre-Jud. $ ',FORMAT(IFNULL(SUM(ABOP),0.0)/1.16,2)),
        CONCAT('Cobros credinomina $ ',FORMAT(IFNULL(SUM(ABOI),0.0)/1.16,2)),
        CONCAT('Cobros cart. dict. $ ',FORMAT(IFNULL(SUM(OTR),0.0)/1.16,2)),
                      '------------------------------------',
        CONCAT('TOTAL:  $ ',FORMAT(IFNULL(SUM(CE+CT+CR+ENG+ABOT+ABOG+ABOM+ABOE+ABOP+ABOI+OTR),0.0)/1.16,2)))
