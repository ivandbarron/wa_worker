...
cat /etc/rsyslog.conf | sed 's/\$ModLoad\ imjournal/\#$ModLoad\ imjournal/g' | sed 's/\$OmitLocalLogging on/\$OmitLocalLogging\ off/g' | sed 's/\$IMJournalStateFile/\#\$IMJournalStateFile/g' > /etc/rsyslog2.conf
echo '*.* @@'$SYSLOG_REMOTE >> /etc/rsyslog3.conf
cat /etc/rsyslog.d/listen.conf | sed 's/\$SystemLogSocketName/\#\$SystemLogSocketName/g' > /etc/rsyslog.d/listen2.conf
#rsyslogd
